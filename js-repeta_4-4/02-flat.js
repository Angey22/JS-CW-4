 ...
 */
console.groupCollapsed('Array.prototype.flat(depth)'); //! Log group name
 ...
 * - Комбинация map + flat
 */
console.groupCollapsed('Array.prototype.flatMap(callback)'); //! Log group name
...
// Особенностью использования метода "flatMap()" является то, что он работает на 1-у вложенность, т.е., при необходимости выполнить операцию "разглаживания" многомерного массива, этот метод может его "разгладить" только на 1-н уровень. В случаях, если нужно выполнить "разглаживание" болье чем на один уровень, рекомендуется использовать не две отдельные функции (как в вышеописанных примерах последовательного использования функций),  а применять "чейнинг" методов для создания единой функции внутри которой используются соответствующие методы - один за другим.


// Создаем функцию на основе "чейнинг" методов
const tags4 = tweets
    // .map(t => t.tags).flat() // 2-а отдельных метода
    .flatMap(t => t.tags) // 1-н "flatMap"
    .reduce((acc, tag) => ({
        ...acc,
        [tag]: acc[tag] ? acc[tag] + 1 : 1,
    }), {}, );
// Выводим в лог значение переменной "tags4"
console.log('Объект-статистика "tags4" =>', tags4);

// Особенностью применения "чейнинга" методов, является то, что можно использовать последовательность из любова количества методов, необходимых для достижения цели. Например, согласно вышеописанного примера, в единой функции на основе "чейнинга", можно использовать как последовательную запись методов "map" и "flat", так и заменяющий их метод "flatMap".
